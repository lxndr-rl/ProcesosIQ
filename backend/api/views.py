...
from .models import Persona, Timeline
        ...
        if(not body):
            ...
            if(not nombre or not apellido or not telefono):
                ...
                Timeline.objects.create(
                    ...
                    descripcion=f'Se creó la persona {nombre} {apellido} con id {Persona.objects.last().id}',
                )
        return response
        ...
        if(id > 0):
            ...
            if(not body):
                ...
                if(len(Personas) > 0):
                    ...
                    Timeline.objects.create(
                        persona=Personas,
                        descripcion=f'Se actualizó la persona {nombre} {apellido} con id {Personas.id}',
                    )
                else:
                    response = JsonResponse(
                        {'status': 'error', 'message': 'No se encontró la persona'}, status=404)
...
class TimelineView(View):
    ...
    def get(self, request):
        # Solo los ultimos 5 registros
        Timelines = list(Timeline.objects.all().order_by('-id')[:5].values())
        if(len(Timelines) > 0):
            response = JsonResponse(
                {'status': 'ok', 'data': Timelines}, status=200)
        else:
            response = JsonResponse(
                {'status': 'error', 'message': 'No se encontraron cambios en los registros'}, status=404)
        return response
